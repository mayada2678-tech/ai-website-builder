...
const identityText=document.createElement("div");const heading=document.createElement("strong");heading.textContent=originalHeading?.textContent||"Kundenservice";const status=document.createElement("span");status.className="customer-chat-status";status.textContent="Schnelle und verlässliche Auskunft";identityText.append(heading,status);identity.append(avatar,identityText);
const close=document.createElement("button");close.type="button";close.className="customer-chat-close";close.setAttribute("aria-label","Chat schließen");close.textContent="×";topbar.append(identity,close);if(originalHeading)originalHeading.remove();panel.prepend(topbar);
